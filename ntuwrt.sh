#!/bin/bash
#ntuwrt.sh是一个openwrt系统用的sh脚本，用以路由器自动登录校园网。
...
#3、上传完毕，按Shift+Ctrl+T打开WinSCP的终端，输入该命令：sed -i 's/exit 0/sh \/root\/ntuwrt.sh \&\nexit 0/' /etc/rc.local
#4、点击执行后，关闭窗口，重启路由器。这样，路由器每次开机就会启动脚本，当网络断开时会自动尝试重连。
#5、若要停用脚本，则打开终端输入：sed -i 's/sh \/root\/ntuwrt.sh \&//' /etc/rc.local  
